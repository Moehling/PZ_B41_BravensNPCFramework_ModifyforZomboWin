...
local function spawnBraven()
    ...
    local braven_parameters = {
        ...
        isEssential = SandboxVars.BB_MyLittleBraven.isEssential, -- 是否为重要角色（默认为false）
		forceStop = SandboxVars.BB_MyLittleBraven.forceStop, -- 强制停止移动（默认false）
        setGod = SandboxVars.BB_MyLittleNPCs.setGod, -- 启用无敌
        outfitName = SandboxVars.BB_MyLittleBraven.outfitName,  -- NPC初始服装（默认为空）
    ...
    local modules = { true, true, true, MyLittleBraven.needCloseDoors, true, true, true, true, true, true }
    ...
    if setupStatus == "CUSTOMIZE" then
        ...
        if MyLittleBraven.outfitName then
            addAndWearItem(inventory, MyLittleBraven.npc, MyLittleBraven.outfitName)
        else
